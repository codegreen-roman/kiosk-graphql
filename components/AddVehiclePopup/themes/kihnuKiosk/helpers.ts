...
interface ParametersForAutomaticConvertation {
  ...
  useContentBefore01march?: boolean
}
export const convertTermsAutoFilled = ({
  attributes,
  coefficientsFromQuery,
  currentLanguage,
  useContentBefore01march = false,
  ...
  return attributes.length
    ? [
        ...
        ...(coefficientsFromQuery
          ? coefficientsFromQuery.map((item) => {
              if (useContentBefore01march) {
                return attributes.find((it) => it.code === `${item}_${currentLanguage}_BEFORE_01.03.2022`)?.value
              }

              return attributes.find((it) => it.code === `${item}_${currentLanguage}`)?.value
            })
          : []),
      ]
    : []
