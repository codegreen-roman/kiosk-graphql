...
export const DialogWrapper = styled.div<{ hasNumPad?: boolean }>`
  display: flex;
  ...
  margin-bottom: ${(props): string => (props.hasNumPad ? '10px' : '0')};
`
...
export const IdNumber = styled.div<{ isSelected?: boolean; isOverview?: boolean }>`
  display: flex;
  justify-content: ${(props): string => (props.isOverview ? 'center' : 'space-between')};
  align-items: center;
  width: 197px;
  height: 70px;
...
export const CompanyBox = styled.div<{ isSelected?: boolean }>`
  padding: 10px;
  margin: 0 0 9px 0;
  display: flex;
  align-items: center;
  justify-content: center;
  border: solid ${(props): string => (props.isSelected ? '2px' : '1px')}
    ${(props): string => (props.isSelected ? contentBlockColors.selectedBorder : contentBlockColors.border)};
  background-color: ${(props): string =>
    props.isSelected ? buttons.button.defaultBackground : contentBlockColors.disabledBackground};
  cursor: pointer;
  p {
    font-family: ${primary};
    color: ${(props): string => (props.isSelected ? textColors.defaultInverted : textColors.heading)};
    font-size: 22px;
    font-weight: 400;
  }
`
